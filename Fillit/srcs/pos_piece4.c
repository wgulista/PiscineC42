...
static int		pos_l1(t_tetri *piece, char **tab, int *i, int j)
{
	if (piece->rotation == 0)
...
static int		pos_l2(t_tetri *piece, char **tab, int *i, int j)
{
	if (piece->rotation == 1)
...
static int		pos_l3(t_tetri *piece, char **tab, int *i, int j)
{
	if (piece->rotation == 2)
		if ((j - (i[0] + 2)) > 0 && (j - (i[1] + 1)))
...
static int		pos_l4(t_tetri *piece, char **tab, int *i, int j)
{
	if (piece->rotation == 3)
		if ((j - (i[0] + 1)) > 0 && i[1] > 1)
			if (tab[i[0]][i[1]] == '.' && tab[i[0] + 1][i[1]] == '.'
